...
var _purr = __webpack_require__(328);
...
console.log(_purr2.default.pluck(person, 'name'));
...
var purr = {
  each: function each(collection, func) {
    func = arguments.length < 2 && this._value ? collection : func;
    collection = arguments.length < 2 && this._value ? this._value : collection;

    var isObject = this.isObject(collection);
    var collectionType = this._checkType(collection);
    var collectionTypeLowerCased = collectionType.toLowerCase();
    var collectionsMap = {
    ...
    var coll = collectionsMap[collectionTypeLowerCased]();
    var i = 0;
    var len = coll.length;

    for (; i < len; i += 1) {
      var current = coll[i];
      var _val = isObject ? collection[current] : current;
      var key = isObject ? current : i;
      func(_val, key, collection, coll);
    }
  ...
  map: function map(collection, func) {
    func = arguments.length < 2 && this._value ? collection : func;
    collection = arguments.length < 2 && this._value ? this._value : collection;
    this._throwIfNotObjectOrArrayOrString(collection);

    var isObject = !this._isArrayOrString(collection);
    var collectionMapped = isObject ? {} : [];

    this.each(collection, function (val, key) {
      var transformed = func(val, key, collection);
  ...
  reduce: function reduce(collection, func, acc) {
    ...
    this.each(collection, function (val, key, collection, coll) {
      var next = void 0;
  ...
  filter: function filter(collection, func) {
    var _this = this;
    ...
    if (isObject) {
      result = {};
      action = function action(to, from, key) {
        to[key] = from[key];
      };
    } else {
      result = [];
      action = function action(to, what) {
        to.push(what);
      };
    }

    this.each(collection, function (val, key) {
      var funcResult = _this.isFunction(func) ? func(val, key, collection) : function () {
        var isObject = _this.isObject(func);
        var funcKey = isObject ? Object.keys(func)[0] : func[0];

        return isObject ? func[funcKey] === val[funcKey] : func[1] === val[funcKey];
      }();
      var condition = reverse ? !funcResult : funcResult;
  ...
  reject: function reject(collection, func) {
    func = arguments.length < 2 && this._value ? collection : func;
    collection = arguments.length < 2 && this._value ? this._value : collection;

    var result = this.filter(collection, func, true);

    return this._isChained ? (this._value = result._value) && this : result;
  },
  all: function all(collection, func) {
    func = arguments.length < 2 && this._value ? collection : func;
    collection = arguments.length < 2 && this._value ? this._value : collection;

    var filtered = this.filter(collection, func);

    filtered = this._isChained ? filtered._value : filtered;

    var result = filtered.length === collection.length;

    return this._isChained ? (this._value = result) && this : result;
  },
  any: function any(collection, func) {
    func = arguments.length < 2 && this._value ? collection : func;
    collection = arguments.length < 2 && this._value ? this._value : collection;
  ...
  find: function find(collection, func) {
    func = arguments.length < 2 && this._value ? collection : func;
    collection = arguments.length < 2 && this._value ? this._value : collection;
  ...
  isTruthy: function isTruthy(thing) {
  ...
  isEmpty: function isEmpty(thing) {
    ...
    var thingType = this._checkType(thing);
    var thingTypeLowerCased = thingType.toLowerCase();
    var resultsMap = {
      object: function object() {
  ...
  contains: function contains(collection, valOrKey) {
    var _this2 = this;
    ...
    var collectionTypeLowerCased = this._checkType(collection).toLowerCase();
    var resultsMap = {
      object: function object() {
      ...
      array: function array() {
        var filtered = _this2.filter(collection, function (item, key) {
          return item === valOrKey;
        });

        filtered = _this2._isChained ? filtered._value : filtered;
    ...
    var result = resultsMap[collectionTypeLowerCased]();

    return this._isChained ? (this._value = result) && this : result;
  },
  destroy: function destroy(collection, key) {
    ...
    var result = !collection[key] ? true : false;
  ...
  groupBy: function groupBy(collection, criteria) {
    criteria = arguments.length < 2 && this._value ? collection : criteria;
    collection = arguments.length < 2 && this._value ? this._value : collection;
    this._throwIfNotObjectOrArrayOrString(collection);

    var result = this.reduce(collection, function (acc, next) {
      var criteriaKey = next[criteria];
  ...
  toArray: function toArray(arrayLike) {
    ...
    if (result.length === 0 && this.isObject(arrayLike)) {
      result = this.reduce(arrayLike, function (acc, next) {
        acc.push(next);
        return acc;
      }, []);
  ...
  debounce: function debounce(fn) {
    ...
    var asap = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  pluck: function pluck(collection, key) {
    key = arguments.length < 2 && this._value ? collection : key;
    collection = arguments.length < 2 && this._value ? this._value : collection;
    ...
    if (this.isArray(collection)) {
      result = this.map(collection, function (val) {
        return val[key];
      });
      result = this._isChained ? result._value : result;
    } else {
      result = this.filter(collection, function (val, valKey) {
        return valKey === key;
      });
      result = this._isChained ? result._value : result;
      result = [result[key]];
  ...
  extendDeep: function extendDeep(collection, source) {
    var _this4 = this;
    ...
    this.each(source, function (val, key) {
      var propType = _this4._checkType(val);

      if (propType === 'Array') {
        collection[key] = [];
        _this4.extendDeep(collection[key], val);
  ...
  clone: function clone(collection) {
    collection = arguments.length < 1 && this._value ? this._value : collection;
    this._throwIfNotOneOfTypes(collection, ['Array', 'Object']);
  ...
  mixin: function mixin(collection) {
    var _this5 = this;
    ...
    collection = arguments.length < 2 && this._value ? this._value : collection;
    this.each(args, function (val, key) {
      _this5._throwIfNotOneOfTypes(val, ['Object']);

      if (key > 0) {
        _this5.extendDeep(collection, val);
      }
    });

    return this._isChained ? (this._value = collection) && this : collection;
  },
  omit: function omit(obj, key) {
  ...
  chunk: function chunk(collection, size) {
    ...
    var result = start ? collection.slice(start, end) : collection.slice(0, end);
  ...
  walk: function walk(collection, func) {
    var _this6 = this;

    this.each(collection, function (val, key) {
      if (_this6.isObject(val) || _this6.isArray(val)) {
        _this6.walk(val, func);
      }
      func(val, key, collection);
    });
  },
  inject: function inject(obj, stuff) {
    var _this7 = this;
    ...
    if (this.isArray(stuff)) {
      this.each(stuff, function (val) {
        _this7.mixin(obj, _defineProperty({}, val, _this7[val]));
      });
    } else {
      this.mixin(obj, _defineProperty({}, stuff, this[stuff]));
    }

    return this._isChained ? (this._value = obj) && this : obj;
  },
  implements: function _implements(obj, methods) {
    this._throwIfNotOneOfTypes(obj, ['Object']);
    this._throwIfNotOneOfTypes(methods, ['Array', 'String']);

    if (this.isArray(methods)) {
      return this.all(methods, function (name) {
        return name in obj;
      });
    }
  ...
  */
  _expose: function _expose(global, item, name) {
    global.__exposed = global.__exposed || {};
    global.__exposed[name] = item;
  },
  _checkType: function _checkType(thing) {
    return Object.prototype.toString.call(thing).replace(/\[|\]|object/g, '').trim();
  },
  _isArrayOrString: function _isArrayOrString(thing) {
    var thingType = this._checkType(thing);

    return thingType === 'Array' || thingType === 'String';
  },
  _throwIfNotObjectOrArrayOrString: function _throwIfNotObjectOrArrayOrString(thing) {
    this._throwIfNotOneOfTypes(thing, ['Object', 'Array', 'String']);
  },
  _throwIfNotObjectOrArray: function _throwIfNotObjectOrArray(thing) {
    this._throwIfNotOneOfTypes(thing, ['Object', 'Array']);
  },
  _throwIfNotOneOfTypes: function _throwIfNotOneOfTypes(thing, types) {
    var thingType = this._checkType(thing);
  ...
  _inspect: function _inspect(collection, deeper) {
    var collectionType = this._checkType(collection);
